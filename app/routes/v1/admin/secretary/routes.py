...
@secretary_bp.route('/<int:item_id>/images', methods=['PUT', 'DELETE'])
@jwt_required()
@intercept_admin_user
def item_image(item_id):
    ...
    try:
        ...
        if request.method == 'PUT':
            item = crud_secretary.put_image(item_id=item_id, path="secretaries")
            return default_return(200, 3, item)

        if request.method == 'DELETE':
            crud_secretary.delete_image(item_id)
